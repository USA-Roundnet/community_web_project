...
const createTournamentMatch = async (tournament_id, matchData) => {
  ...
  const [createdMatchId] = await knex("Series").insert(insertPayload);

  const createdMatch = await getTournamentMatchById(tournament_id, createdMatchId);
  if (!createdMatch) {
...
const saveTournamentMatchResults = async (
  ...
  if (payload.games.length !== 1) {
    throw new BadRequestError("Only single-game results are supported");
  }
  ...
  const submittedGame = payload.games[0] || {};
  const gameNumber = Number(submittedGame.game_number || 1);
  if (gameNumber !== 1) {
    throw new BadRequestError("Single-game matches must use game_number 1");
  }

  const normalizedGame = {
    game_number: 1,
    ...validateCompletedGameScore(
      submittedGame.team1_score,
      submittedGame.team2_score,
      scoreRules
    ),
  };

  const winnerRegistrationId =
    normalizedGame.winner_side === 1
      ? match.registration1_id
      : match.registration2_id;
  const team1Wins = normalizedGame.winner_side === 1 ? 1 : 0;
  const team2Wins = normalizedGame.winner_side === 2 ? 1 : 0;
  ...
  await knex.transaction(async (trx) => {
    ...
    await trx("Game").insert({
      series_id: match.id,
      game_number: normalizedGame.game_number,
      team1_score: normalizedGame.team1_score,
      team2_score: normalizedGame.team2_score,
      status: "completed",
    });
...
const getTournamentOutcomeStats = async (tournament_id) => {
  ...
  matches.forEach((match) => {
    ...
    const registrationOneSeed = registrationOneStats.seed;
    const registrationTwoSeed = registrationTwoStats.seed;
